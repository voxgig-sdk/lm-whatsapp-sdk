...
function isRunnable(code) {
    return (/new\s+LmWhatsappSDK\b/.test(code) ||
        /\bLmWhatsappSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+LmWhatsappSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/LmWhatsappSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
