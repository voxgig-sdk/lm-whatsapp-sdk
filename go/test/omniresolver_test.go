...
import (
	...
	omni "github.com/voxgig-sdk/lm-whatsapp-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LmWhatsappError); is {
				out := map[string]any{
					"name":    "LmWhatsappError",
...
func omniCtx(arg any, client *sdk.LmWhatsappSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LmWhatsappSDK); is {
			client = live
