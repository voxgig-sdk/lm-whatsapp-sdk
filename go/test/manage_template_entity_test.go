...
func TestManageTemplateEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMWHATSAPP_TEST_MANAGE_TEMPLATE_ENTID JSON to run live")
			return
		}
...
func manage_templateBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMWHATSAPP_TEST_MANAGE_TEMPLATE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LMWHATSAPP_TEST_MANAGE_TEMPLATE_ENTID"])
