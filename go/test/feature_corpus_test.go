...
//
// NewLmWhatsappSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.LmWhatsappSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewLmWhatsappSDK(opts)
}
...
func fcCandidates(client *sdk.LmWhatsappSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.LmWhatsappSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.LmWhatsappSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.LmWhatsappError
