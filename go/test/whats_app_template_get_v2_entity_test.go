...
func TestWhatsAppTemplateGetV2Entity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__ENTID JSON to run live")
			return
		}
		client := setup.client
...
func whats_app_template_get_v2BasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__ENTID": idmap,
		"LMWHATSAPP_TEST_LIVE":      "FALSE",
		"LMWHATSAPP_TEST_EXPLAIN":   "FALSE",
		"LMWHATSAPP_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__ENTID"])
