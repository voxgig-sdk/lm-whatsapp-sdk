...
func TestMediaEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMWHATSAPP_TEST_MEDIA_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		mediaRef01Data = core.ToMapAny(mediaRef01DataResult)
...
func mediaBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMWHATSAPP_TEST_MEDIA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LMWHATSAPP_TEST_MEDIA_ENTID"])
