...
func TestWhatsAppTemplateGetV2PaginationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__PAGINATION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		whatsAppTemplateGetV2PaginationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.whats_app_template_get_v2_pagination", setup.data)))
...
func whats_app_template_get_v2_paginationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__PAGINATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__PAGINATION_ENTID"])
