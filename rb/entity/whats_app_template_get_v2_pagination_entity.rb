...
class WhatsAppTemplateGetV2PaginationEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "whats_app_template_get_v2_pagination"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    WhatsAppTemplateGetV2PaginationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = LmWhatsappHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [WhatsAppTemplateGetV2Pagination, Hash] the current WhatsAppTemplateGetV2Pagination data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = LmWhatsappHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of WhatsAppTemplateGetV2Pagination fields)
  def match_get
  ...
  #
  # @param reqmatch [WhatsAppTemplateGetV2PaginationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.WhatsAppTemplateGetV2Pagination.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [WhatsAppTemplateGetV2Pagination, Hash] the loaded WhatsAppTemplateGetV2Pagination; raises LmWhatsappError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = LmWhatsappHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
