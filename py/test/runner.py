...
class LmWhatsappTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            LmWhatsappTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = LmWhatsappTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = LmWhatsappTestRunner.getenv("LM_WHATSAPP_TEST_LIVE")
        override = LmWhatsappTestRunner.getenv("LM_WHATSAPP_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = LmWhatsappTestRunner.getenv(key)
        ...
        explain = LmWhatsappTestRunner.getenv("LM_WHATSAPP_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if LmWhatsappTestRunner._test_control is not None:
            return LmWhatsappTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                LmWhatsappTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            LmWhatsappTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = LmWhatsappTestRunner.load_test_control()
...
def load_env_local():
    LmWhatsappTestRunner.load_env_local()


def env_override(m):
    return LmWhatsappTestRunner.env_override(m)


def entity_data(v):
    return LmWhatsappTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return LmWhatsappTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return LmWhatsappTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return LmWhatsappTestRunner.load_test_control()


def live_client_options():
    return LmWhatsappTestRunner.live_client_options()


def live_delay_ms():
    return LmWhatsappTestRunner.live_delay_ms()
