...
class ManageTemplateEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ManageTemplateEntity(self._client, opts)
    ...
    def data_get(self) -> ManageTemplate:
    ...
    def match_get(self) -> ManageTemplate:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ManageTemplate:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ManageTemplate().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
