...
[MCP](https://modelcontextprotocol.io) server exposing the LmWhatsapp SDK as
two agent tools — `lm-whatsapp_list` and `lm-whatsapp_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lm-whatsapp-mcp)
make build
...
claude mcp add --scope user lm-whatsapp \
  -- /absolute/path/to/lm-whatsapp-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./lm-whatsapp-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// lm-whatsapp_list: first page of records
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lm-whatsapp-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user lm-whatsapp \
     -- "$PWD"/dist/*/lm-whatsapp-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `lm-whatsapp_list` and `lm-whatsapp_load` tools now appear
   in new sessions. Ask the agent to *"list manage_template using lm-whatsapp"*
   and it calls `lm-whatsapp_list` with `{"entity":"manage_template"}`.
...
### Call the `lm-whatsapp_list` tool
...
### Call the `lm-whatsapp_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `lm-whatsapp_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `lm-whatsapp_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LM_WHATSAPP_APIKEY` | API key sent with every request. |
| `LM_WHATSAPP_BASE` | Optional override of the API base URL. |
...
```sh
./lm-whatsapp-mcp -transport http -addr :18080 &
