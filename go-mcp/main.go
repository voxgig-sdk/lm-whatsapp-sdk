// Command LmWhatsapp-mcp is an MCP server exposing the LmWhatsapp
// SDK as MCP tools.
//
// Two tools are registered:
//
//	LmWhatsapp_list  — list records of an entity
//	LmWhatsapp_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: LM_WHATSAPP_APIKEY carries the API key and
	// LM_WHATSAPP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LM_WHATSAPP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LM_WHATSAPP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLmWhatsappSDK(opts)
