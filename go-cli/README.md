...
boru-driven command-line client **and** interactive REPL for the LmWhatsapp
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lm-whatsapp-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lm-whatsapp-cli
   ```
...
Run with no arguments to open a REPL (prompt `lm-whatsapp>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `manage_template`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/lm-whatsapp-cli`. |
...
(`./lm-whatsapp-cli <expr>`) and interactively (the REPL), and expressions compose the
