...
module LmWhatsappUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = LmWhatsappResponse.new({})
    ...
    if fetch_err
      ...
      response = LmWhatsappResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LmWhatsappResponse.new(fetched)
    else
