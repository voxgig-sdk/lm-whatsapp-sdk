...
module LmWhatsappUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = LmWhatsappContext.new({}, nil)
    end
    op = ctx.op || LmWhatsappOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || LmWhatsappResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(LmWhatsappError) ? err.msg : err.to_s
    ...
    sdk_err = LmWhatsappError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(LmWhatsappError)
