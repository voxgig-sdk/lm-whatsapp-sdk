...
-- by passing it.
function LmWhatsappSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function LmWhatsappSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "LmWhatsappSDK: graphql: " .. msg
    res.graphql = errors
  end
