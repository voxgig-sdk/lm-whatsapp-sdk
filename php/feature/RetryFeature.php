...
class LmWhatsappRetryFeature extends LmWhatsappBaseFeature
{
    ...
    public function init(LmWhatsappContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (LmWhatsappContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(LmWhatsappContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(LmWhatsappContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
