...
class LmWhatsappSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = LmWhatsappHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = LmWhatsappHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!LmWhatsappFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, LmWhatsappFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return LmWhatsappUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = LmWhatsappHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = LmWhatsappHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = LmWhatsappHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new LmWhatsappError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = LmWhatsappHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new LmWhatsappError("graphql_error",
                "LmWhatsappSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_manage_template = null;

    // Canonical facade: $client->ManageTemplate()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->manage_template()
    // resolves here too.
    public function ManageTemplate($data = null)
    {
        require_once __DIR__ . '/entity/manage_template_entity.php';
        if ($data === null) {
            if ($this->_manage_template === null) {
                $this->_manage_template = new ManageTemplateEntity($this, null);
            }
            return $this->_manage_template;
        }
        return new ManageTemplateEntity($this, $data);
    }
    ...
    private $_whats_app_template_get_v2 = null;

    // Canonical facade: $client->WhatsAppTemplateGetV2()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->whats_app_template_get_v2()
    // resolves here too.
    public function WhatsAppTemplateGetV2($data = null)
    {
        require_once __DIR__ . '/entity/whats_app_template_get_v2_entity.php';
        if ($data === null) {
            if ($this->_whats_app_template_get_v2 === null) {
                $this->_whats_app_template_get_v2 = new WhatsAppTemplateGetV2Entity($this, null);
            }
