...
// LmWhatsapp SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class LmWhatsappMakePoint
{
    public static function call(LmWhatsappContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof LmWhatsappError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = LmWhatsappHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = LmWhatsappHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
