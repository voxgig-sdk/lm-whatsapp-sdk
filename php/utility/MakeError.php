...
// LmWhatsapp SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class LmWhatsappMakeError
{
    public static function call(?LmWhatsappContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new LmWhatsappContext([], null);
        }
        $op = $ctx->op ?? new LmWhatsappOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new LmWhatsappResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof LmWhatsappError) ? $err->msg : (string)$err;
        $msg = "LmWhatsappSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new LmWhatsappError('', $msg, $ctx);
        ...
        if ($err instanceof LmWhatsappError) {
