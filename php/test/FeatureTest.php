...
class FtCtrl extends LmWhatsappControl
{
...
class FtClient
{
    ...
    private LmWhatsappUtility $utility;

    public function __construct(LmWhatsappUtility $utility, array $options)
    {
    ...
    public function get_utility(): LmWhatsappUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LmWhatsappContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public LmWhatsappContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LmWhatsappContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LmWhatsappConfig::make_feature($name);
    ...
    private static function buildUrl(LmWhatsappSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LmWhatsappError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LmWhatsappResponse($fetched);
        }
    ...
    private function fail(LmWhatsappContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LmWhatsappContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
