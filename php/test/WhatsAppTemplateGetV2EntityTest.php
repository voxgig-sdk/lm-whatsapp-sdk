...
class WhatsAppTemplateGetV2EntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function whats_app_template_get_v2_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["LMWHATSAPP_TEST_WHATS_APP_TEMPLATE_GET_V__ENTID"]);
